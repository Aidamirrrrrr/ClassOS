...
## Проверка готовности к прогону 2026-09-05

Отдельный проход перед началом runtime-приёмки: не «работает ли код», а
«можно ли вообще начать прогон по [RUNTIME_ACCEPTANCE.md](RUNTIME_ACCEPTANCE.md)».
Нашлось три препятствия, каждое из которых остановило бы прогон, и ни одно из
них не было бы видно ни сборкой, ни тестами — они лежали в эксплуатации, а не
в логике.

| Препятствие | Чем грозило | Состояние |
| --- | --- | --- |
| Реестр устройств консоли жил в памяти, а зарегистрированное устройство второй раз enrollment не проходит | Перезапуск консоли отключал класс без пути восстановления по сети: обход всех машин руками | Исправлено, реестр на диске |
| Адрес устройства не обновлялся из обнаружения | После смены IP устройство видно в списке, но недостижимо | Исправлено |
| Продуктовый установщик не ограничивал права на каталог установки | Ученик мог подменить бинарник службы, то есть получить выполнение от LocalSystem | Исправлено |
| В Cloud нет способа создать первого владельца | Блок 12.2 не с чего начать: база пуста, регистрации в API нет | Добавлен `scripts/bootstrap.ts` |
| Окно enrollment не аутентифицировано | Чужая машина в сети перехватывает устройство, подключившись первой | Решение в ADR-0018, протокол не реализован |
| Подпись кода | Пункты §10 непроходимы | Требует покупки сертификата, не кода |

Первые четыре строки прогон разблокируют. Пятая его не блокирует — прогон
идёт в контролируемой сети — но остаётся непройденным пунктом security-
чеклиста. Шестая по-прежнему закрывает путь к первому платному филиалу.

Ни одна строка не меняет `Runtime validation`: исправленный код по-прежнему
ни разу не запускался в классе.
