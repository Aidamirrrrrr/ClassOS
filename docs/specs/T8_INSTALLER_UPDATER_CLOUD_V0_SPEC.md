...
**Первый Owner заводится при развёртывании, а не через API.** Всякая роль в этой матрице выдаётся уже существующим Owner/Admin, поэтому в пустой базе выдать её некому, а маршрут, создающий Owner без аутентификации, был бы обходом всей матрицы. Организация и первый владелец создаются `services/cloud/scripts/bootstrap.ts` тем, у кого есть доступ к базе; членство первого владельца имеет скоуп на всю организацию (`branch_id IS NULL`), иначе он не смог бы создать первый филиал.
