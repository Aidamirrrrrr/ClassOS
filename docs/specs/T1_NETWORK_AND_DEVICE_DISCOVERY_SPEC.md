...
`controlPort`, объявленный в discovery announcement — конкретное значение выбрать при реализации и задокументировать в README-T1.md (не хардкодить магическое число без объяснения, не конфликтовать с уже занятыми Windows-портами).
