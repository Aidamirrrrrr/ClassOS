...
> **Процесс проверки обновлён 2026-09-04.** Реализация и runtime validation
> теперь учитываются отдельно согласно `BACKLOG.md`. После завершения кода,
> тестов, lint, Windows cross-check, cross-build и CI разрешено начать T1 с
> пометкой T0 `Runtime validation: CI only`. Это не отменяет и не засчитывает
> реальные acceptance tests из этого документа; их результаты фиксируются
> только после запуска на постоянной Windows 11 машине.
