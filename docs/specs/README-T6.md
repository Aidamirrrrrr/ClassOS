...
Allowlist генерировался правилами `Path="*\Code.exe"` — совпадение по имени
файла в любом каталоге. Под standard-user это обходится тривиально:
переименовать программу в `Code.exe`, положить в свой профиль и запустить.
То есть главный пункт DoD (§2) не выполнялся бы даже на исправно работающем
AppLocker.

Теперь разрешения действуют только в `%PROGRAMFILES%` и `%WINDIR%`, а запреты
— по-прежнему везде (ADR-0017). Следствие: приложение, установленное в
профиль пользователя, политикой не разрешается, поэтому управляемые пакеты
ставятся `winget --scope machine`.

## Известные ограничения

- Publisher-правила прочнее path-правил и остаются целью, но требуют subject
  сертификатов, снятых с настоящих подписанных бинарников; переход на них —
  отдельный ADR после первой приёмки на оборудовании (ADR-0017).
