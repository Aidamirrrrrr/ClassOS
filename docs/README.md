...
Сейчас: **реализация T0 в процессе, спецификации T0–T8 готовы**. Код T0 проходит локальные проверки, Windows cross-build и CI; полная проверка на постоянной Windows 11 машине отложена и учитывается отдельным статусом. После завершения автоматических проверок T0 разрешено последовательно начать T1, не выдавая T0 за прошедший runtime acceptance (см. `CLAUDE.md`, раздел «Правило последовательных milestone»).
