...
## Правило последовательных milestone

Одновременно реализуется **только текущий** milestone из `docs/specs/BACKLOG.md`. Следующий milestone можно начать после завершения кода и автоматических проверок текущего, даже если реальная Windows-приёмка отложена. Непроверенные сценарии обязательно фиксируются в `Runtime validation` и отчёте `docs/specs/README-T<N>.md`. Каждый spec явно перечисляет Non-goals/Out of scope — это обязательная часть контракта, а не пожелание.
...
Новые внешние Rust-зависимости добавляются только через `cargo add`, а удаляются
через `cargo rm`. Версии внешних библиотек не вписываются и не подбираются
вручную в `Cargo.toml`; итоговые manifest и lockfile проверяются после команды.
Локальные workspace-зависимости также добавляются через `cargo add --path`.

## Язык проекта

Инженерные комментарии, rustdoc, сообщения для разработчика, README и внутренняя документация пишутся на русском языке. Идентификаторы кода, имена типов и функций, названия Win32 API, поля протокола и стабильные имена событий логирования остаются на английском, чтобы их можно было искать и сопоставлять с документацией платформы. Пользовательские сообщения продукта локализуются отдельно на уровне UI.
